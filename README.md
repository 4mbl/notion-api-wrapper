...
3. Make the secret key available as an environment variable under the name `NOTION_API_KEY` (or `NOTION_TOKEN`). You may also pass it as a parameter to the query functions. You should also store the database ID in an environment variable but this is not mandatory. The examples below assume that you have done so.
...
```ts
const db = new DatabaseIterator(process.env.NOTION_DATABASE_ID, {
  batchSize: 10,
  yieldSize: 2,
});
